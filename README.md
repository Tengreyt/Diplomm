...
- `POST /attempts` - создать серверную попытку и получить урок
  body: `difficulty`; для AI-задания дополнительно `targetText`; ответ также содержит heatmap ошибок клавиатуры
...
- `GET /progress` - сводка, серия, достижения и 10 последних тренировок
- `GET /progress/history` - cursor-пагинация полной истории тренировок
- `GET /progress/history/:resultId` - подробности отдельной тренировки
- `POST /progress/keyboard-heatmap/reset` - начать новый период анализа ошибок клавиатуры без удаления истории
...
Повторные переходы между разделами профиля не создают лишние запросы:
участники кланов, рейтинг, прогресс и AI-совет кэшируются на клиенте с TTL.
После тренировки или logout связанные записи адресно инвалидируются.
